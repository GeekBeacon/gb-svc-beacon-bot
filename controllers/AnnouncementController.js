...
module.exports = {
    crudHandler: async function(message, args, client) {
        ...
        async function createAnnounce() {
            ...
            // Check if the secret emojirole 2nd arg was given
            if(args.length === 2 && args[1].toLowerCase() === "emojirole") {
                message.channel.send(`Dix`)
                ...
                // Ask for the emoji to use for each role

            // If the secret arg isn't given then proceed as normal
            } else {
                let announcement = {
                    title: null,
                    body: null,
